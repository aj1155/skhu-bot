...
public class KoreanAnalysisService {
    ...
    public List<String> tokenizeKoreanNoun(String text){
        ...
        return list.stream().filter(data -> data.toString().contains("Noun") || data.toString().contains("Number") || data.toString().contains("ProperNoun")).map(KoreanTokenJava::getText).collect(Collectors.toList());
